...
impl FileDiffViewer {
    ...
    fn draw(&mut self, f: &mut Frame) {
        ...
        if self.cursor_pos < self.diffs.len() {
            ...
            let info_text = Text::from(Span::from(format!("Posição: {:08x}", offset)));
            let info_paragraph = Paragraph::new(info_text)
                .block(Block::default().borders(Borders::ALL).title("Informação"));
...
//The byte_style dosen't work at custom terminal
